...
export class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  static categorizeError(error: Error): ErrorCategory {
    ...
    if (message.includes('network') || message.includes('fetch') || 
        message.includes('load') || message.includes('timeout')) {
      return 'network';
    }
    ...
    if (message.includes('cannot read') || message.includes('undefined') ||
        message.includes('null') || message.includes('reference')) {
      return 'runtime';
    }
  ...
  private handleReportBug = (): void => {
    ...
    console.log('Bug report data:', errorReport);
  ...
  render(): React.ReactNode {
    if (this.state.hasError) {
      ...
      return (
        <SacredCard variant="solid" className="p-8 my-6 border-l-4 border-l-red-500">
          <div className="text-center">
            ...
            {this.state.errorId && (
              <div className="mt-6 pt-4 border-t border-gray-200">
                <p className="text-xs text-gray-500">
                  Error ID: <code className="font-mono bg-gray-100 px-2 py-1 rounded">{this.state.errorId}</code>
                </p>
              </div>
            )}
            ...
            {(process.env.NODE_ENV === 'development' || this.props.showErrorDetails) && this.state.error && (
              <details className="mt-6 text-left">
                <summary className="cursor-pointer text-gray-600 font-medium text-sm hover:text-gray-800 transition-colors">
                  🔧 Error Details (Development)
                </summary>
                <div className="mt-3 p-4 bg-gray-50 rounded-lg border">
                  <div className="space-y-3">
                    <div>
                      <h4 className="text-sm font-semibold text-gray-700 mb-1">Error:</h4>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-xs">
                      ...
                      <div>
                        ...
                        <span className="ml-2">{this.state.retryCount}/{maxRetries}</span>
                      </div>
                    </div>
                  </div>
                </div>
...
export const InlineErrorFallback: React.FC<{ message?: string }> = ({ 
  ...
  <div className="inline-flex items-center text-red-600 bg-red-50 px-3 py-2 rounded-lg text-sm">
